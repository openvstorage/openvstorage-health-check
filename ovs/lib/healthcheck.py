...
class HealthCheckController:
    ...
    def execute_check():
        ...
        try:
            ...
            LOGGER.success("Connection to etcd succeeded!", "etcd")
        except EtcdConnectionFailed:
            LOGGER.exception("Connection to etcd failed!", "etcd")
        ...
        if platform == 0:
            ...
            raise UnsupportedPlatformException(Exception("Platform '{0}' is CURRENTLY NOT supported".format(platform)))
    ...
    def get_results():
        ...
        LOGGER.success("SUCCESSFULL={0} FAILED={1} SKIPPED={2} WARNING={3} EXCEPTION={4}"
                       .format(LOGGER.HC_success, LOGGER.HC_failure, LOGGER.HC_skip, LOGGER.HC_warning,
                               LOGGER.HC_exception), 'exception_occured', False)
